...
$halaman = "surat_pilih"; // halaman
$dataapa = "Pilih Transaksi"; // data
...
if ($chmod >= 2 || $_SESSION['jabatan'] == 'admin') {
  ...
  <!-- KONTEN BODY AWAL -->
      ...
      <div class="box">
        <div class="box-header with-border">
          <h3 class="box-title">Pilih Transaksi</h3>
 ...
 <div class="form-group">
                ...
                <select class="form-control select2" style="width: 100%;" name="search" required>
                   ...
                   <?php
        $sql=mysqli_query($conn,"select * from stok_keluar order by no desc");
        while ($row=mysqli_fetch_assoc($sql)){
          if ($nota==$row['nota'])
          echo "<option value='".$row['nota']."' selected='selected'>".$row['tgl']." | ".$row['nota']."</option>";
          else
          echo "<option value='".$row['nota']."'>".$row['tgl']." | ".$row['nota']."</option>";
        }
      ?>
                </select>
              </div>
